...
const { response } = require('./voice-control-files/second-server')
let close

app.use(express.urlencoded({extended: true}))
app.use(express.json())

Promise.allSettled([obs(app), voicemeeter(app), keypress(app), voiceControl(app)]).then((results) => {
        ...
        axios.post('http://localhost:23708/obs/sendRequest/GetSceneList')
            .then(res => {
                ...
                if(v.validate(req.body, schema).errors.length == 0){
                    ...
                    fs.writeFileSync('server-config.json', JSON.stringify(serverConfig))
                }else{
                    resp.status(400)
                    resp.send(JSON.stringify(v.validate(req.body, schema).errors[0]))
                }
